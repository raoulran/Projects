...
from hand_clap_detect import detect_hand_claps
...
def volume():
    ...
    while True:
        ...
        if len(lmList) != 0:
            ...
            xl, yl = lmList[4][1],lmList[4][2]
            x2, y2 = lmList[8][1],lmList[8][2]
            cx, cy = (xl+x2)//2, (yl+y2)//2
            
            cv2.circle(img, (xl,yl), 10, (255, 0, 225), cv2.FILLED)
            cv2.circle(img, (x2,y2), 10, (255, 0, 225), cv2.FILLED)
            cv2.line(img, (xl,yl), (x2,y2), (255, 0, 255), 3)
            cv2.circle(img, (cx,cy), 10, (255, 0, 225), cv2.FILLED)
        
            length = math.hypot(x2-xl, y2 - yl)
            print(length)
